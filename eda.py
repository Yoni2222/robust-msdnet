...
# Helper to decide x-ticks (names or numbers)
def set_xticks(dataset_name, x, classes):
    if dataset_name == "cifar100":
        plt.xticks(x, [str(i) for i in range(len(classes))])
        plt.xlabel("Class Index (0-99)")
    else:
        plt.xticks(x, classes, rotation=45, ha='right')

# ---------- 2. Class Distribution ----------
def class_distribution(data, classes, dataset_name, title="Class Distribution"):
    counts = defaultdict(int)
    for _, label in data:
        counts[label] += 1
    ...
    set_xticks(dataset_name, x, classes)
    plt.ylabel("Percentage (%)")
    plt.title(title)
    plt.show()

# ---------- 3. Color Distribution per Class ----------
def color_distribution(data, classes, dataset_name):
    color_sums = {cls: [0, 0, 0] for cls in range(len(classes))}
    counts = defaultdict(int)
...
# ---------- 4. Mean & Variance per Class ----------
def mean_variance_per_class(data, classes, dataset_name):
    means, variances = [], []
    for i in range(len(classes)):
        cls_pixels = [img.mean().item() for img, label in data if label == i]
        means.append(np.mean(cls_pixels))
...
def mean_intensity_per_class(data, classes, dataset_name):
    intensities = []
    for i in range(len(classes)):
        cls_vals = []
        for img, label in data:
            if label == i:
                gray_vals = img.mean(dim=0).mean().item()
                cls_vals.append(gray_vals)
        intensities.append(np.mean(cls_vals))

    x = np.arange(len(classes))
    plt.figure(figsize=(12, 6))
    plt.bar(x, intensities)
    set_xticks(dataset_name, x, classes)
    plt.ylabel("Mean Intensity")
    plt.title("Average Pixel Intensity per Class")
    plt.show()
...
# ---------- 6. Run EDA ----------
if __name__ == "__main__":
    dataset_name = "cifar100"  # change to "cifar10" if needed
    train_data, class_names = load_cifar(dataset_name, train=True)

    class_distribution(train_data, class_names, dataset_name, f"{dataset_name.upper()} Class Distribution")
    color_distribution(train_data, class_names, dataset_name)
    mean_variance_per_class(train_data, class_names, dataset_name)
    mean_intensity_per_class(train_data, class_names, dataset_name)
